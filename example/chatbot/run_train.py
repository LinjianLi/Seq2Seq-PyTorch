...
(train_data, val_data, test_data) = get_train_val_data(data_file="./data/formatted_movie_lines.txt", vocab_eng=vocab_eng)
...
for data in random.choices(test_data, k=5):
    inp, target = data["input"], data["target"]
    infer = model.infer(inp, max_length=20)

    inp = " ".join(vocab_eng.sentence_from_indexes(inp))
