...
class BeamScorer(ABC):
    ...
    """
    The functions `process` and `finalize` are stateless. So, the input of the
    function should include the previous states.
...
class BeamHypotheses:
    ...
    """
    The advantage of designing a BeamHypotheses class as the container of
    hypotheses is that we can encapsulate some processes in the class methods,
    which makes the management of hypotheses simpler.

    For instance, when adding a new hypothesis, we need to check whether it is
    worse than any hypothesis that already exists and the number of existing
    hypotheses is full (beam width). If true, then we should not add this new
    hypothesis. If false, we can add this hypothesis, and may need to remove
    the worst hypothesis if the container is full.
    """
    ...
    def __init__(self,
        ...
        """
        self.max_length = max_length - 1  # ignoring bos_token (beginning of sentence)
        self.length_penalty = length_penalty
        self.early_stopping = early_stopping
        self.num_beams = num_beams  # A.K.A. beam width
        self.beams = []
    ...
    def __repr__(self):
        ...
        main_string = ""
        for b in self.beams:
            main_string += str(b[0]) + " " + str(b[1].tolist()) + "\n"
        main_string += "Worst score: {}".format(self.worst_score)
        return main_string
    ...
    def add(self, hyp: torch.LongTensor, sum_logprobs: float):
        ...
        """
        # The mathematical equation for the score of a hypothesis is
        # ```latex
        # $$score(Y|X) = \frac{log P(Y|X)}{(|Y|)^{\alpha}}$$
        # where $\alpha$ is a hyperparameter of length penalty
        # ````
        score = sum_logprobs / (hyp.shape[-1] ** self.length_penalty)
        if len(self) < self.num_beams or score > self.worst_score:
            # If the container is not full or the new hypothesis is better
            # than the worst hypothesis in the container, we can add this new
            # hypothesis into the container. Otherwise, we just ignore it.
